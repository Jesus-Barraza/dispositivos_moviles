...
export const Buscar = () => {
    ...
    const productoConsultaNombre = async () => {
        try {
            const nombre1 = name.trim() === "" ? "" : name;

            const response = await axios.get(`http://10.0.2.2:5000/producto/${nombre1}`);

            if (response.data.status == 200) {
                if (response.data.result.length > 0) {
                    const first = response.data.result[0];

                    console.log("Tipo imagen:", typeof first.imagen);
                    console.log("Base64 preview:", first.imagen?.slice(0, 50));
    ...
    const renderItem = ({ item }: { item: productoEstructura }) => (
        <View style={styles.card}>
            ...
            {item.imagen ? (
                <Image
                    source={{
                        uri: `data:image/${item.imagen && item.imagen.startsWith("/9j/") ? "jpeg" :
                    ...
                    <View style={styles.modalContainer}>
                        <ScrollView>
                            ...
                            {producto.imagen ? (
                                <Image
                                    source={{
                                        uri: `data:image/${producto.imagen.startsWith("/9j/") ? "jpeg" :
